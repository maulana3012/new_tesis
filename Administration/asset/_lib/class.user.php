...
class USER 
{
   ...
   public function true_login($femail, $expired){
    ...
    public function register_access($fnik,$fullname,$femail)
    {
        try{
            $tmppassword = "Qwerty123!~";
            $fstatus = 2;
            $newPassword = password_hash($tmppassword, PASSWORD_DEFAULT);
            $stmt = $this->db->prepare("INSERT INTO tb_access_karyawan (
            NIK,
            FULLNAME,
            TEMPORARY_PASSWORD,
            EMAIL,
            ...
            $stmt->bindparam(":tmppassword", $newPassword);
            $stmt->bindparam(":femail", $femail);
            $stmt->bindparam(":fstatus", $fstatus);
    ...
    public function data_image($fid){
        $stmt = $this->db->prepare("SELECT * FROM tb_image WHERE ID_KARYAWAN=:fid");
        $stmt->execute(array(":fid"=>$fid));
        $data = $stmt->fetchAll();
        return $data;
    }

    public function upload_register_employe($fKid,$fnameKaryawan,$localPath,$status){
        ...
        try{
            ...
            $stmt->bindparam(":fimage_name", $fnameKaryawan);
            $stmt->bindparam(":fpath_rgb", $localPath);
